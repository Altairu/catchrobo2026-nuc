...
class SoundNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('speech_speed', 5.0)  # 話速 (1.0 = 標準)
        self.declare_parameter('enc_timeout', 1.5)    # エンコーダ途絶判定時間 (秒)
        ...
        self.enc_timeout = self.get_parameter('enc_timeout').get_parameter_value().double_value

        # 音声モデルの自動検索・適用 (東北大学 tohoku-f01 を最優先)
        voice_candidates = [
            # 1. 東北大学モデル (落ち着いた女性アナウンス)
            Path(__file__).resolve().parent / 'voice' / 'tohoku-f01-neutral.htsvoice',
            Path('/home/altair/catchrobo2026-nuc/src/catchrobo_nuc/catchrobo_nuc/voice/tohoku-f01-neutral.htsvoice'),
            # 2. Mei (通常女性声)
        ...
        for candidate in voice_candidates:
            if candidate.exists():
                ...
                self.get_logger().info(f'音声モデルを適用: {self.voice_model}')
                break

        # ─── 状態管理（前回状態の保持・チャタリング防止） ───
        self.last_can_connected = None       # USB-CAN変換器の接続状態
        self.last_mcu_connected = None       # マイコン(MDD1)通信状態 (エンコーダ受信判定)
        self.last_serial_connected = None    # ロボマスモーターの接続状態
    ...
    def _on_can_status(self, msg: String):
        """CAN通信状態（USB-CAN変換器およびマイコンエンコーダ受信）の変化を検知"""
        try:
            data = json.loads(msg.data)
            now = time.time()

            # ── 1. USB-to-CAN 変換器の接続状態 ──
            adapter_connected = bool(data.get('connected', False))

            if self.last_can_connected is None:
                self.last_can_connected = adapter_connected
                if adapter_connected:
                    self.speak('キャン変換器、接続完了')
            elif adapter_connected != self.last_can_connected:
                self.last_can_connected = adapter_connected
                if adapter_connected:
                    self.speak('キャン変換器を接続しました')
                else:
                    self.speak('警告、キャン変換器が切断されました')

            # ── 2. 実機マイコン (MDD1) とのCAN通信状態 (エンコーダ受信判定) ──
            modules = data.get('modules', {})
            mdd1 = modules.get('MDD1', {})
            enc_last_update = float(mdd1.get('enc_last_update', 0.0))

            # 直近0.5秒以内にエンコーダデータを受信していればマイコン通信中と判定
            mcu_connected = adapter_connected and (enc_last_update > 0) and ((now - enc_last_update) < self.enc_timeout)

            if self.last_mcu_connected is None:
                if mcu_connected:
                    self.last_mcu_connected = True
                    self.speak('マイコンキャン通信、接続完了')
                elif adapter_connected and (now - self.state_init_time > 3.0):
                    self.last_mcu_connected = False
            else:
                if mcu_connected != self.last_mcu_connected:
                    self.last_mcu_connected = mcu_connected
                    if mcu_connected:
                        self.speak('マイコンキャン通信、接続完了')
                    else:
                        # 変換器自体の切断と重複しないよう配慮
                        if adapter_connected:
                            self.speak('警告、マイコンキャン通信が途絶しました')

        except Exception as e:
            self.get_logger().error(f'CANステータス解析エラー: {e}')

    def _on_serial_status(self, msg: String):
        """ロボマスモーターシリアル通信状態の変化を検知"""
        try:
            data = json.loads(msg.data)
            ...
            if self.last_serial_connected is None:
                ...
                if connected:
                    self.speak('ロボマスモーター通信、接続完了')
                return

            if connected != self.last_serial_connected:
                self.last_serial_connected = connected
                if connected:
                    self.speak('ロボマスモーター通信を接続しました')
                else:
                    self.speak('警告、ロボマスモーター通信が切断されました')
        except Exception as e:
